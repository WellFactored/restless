...
class Selection$Spec extends FlatSpec with Matchers {
  ...
  it should "project the right fields" in {
    val foos = (1 to 50).map(Foo(_, Some("s"), Some(3)))
    val projection = List(Path(List("id")), Path(List("s")))
    val expected = (1 to 50).map(Foo(_, Some("s"), None)).map(Json.toJson(_))

    selectJson(foos, None, Some(projection), None)(_.id) shouldBe expected
  }

  it should "filter empty objects" in {
    val foos = (1 to 10).map(i => Foo(i, if (i % 2 == 0) Some("s") else None, None))
    val projection = List(Path(List("s")))
    val expected = (1 to 10).flatMap(i => if (i % 2 == 0) Some(JsObject(Seq("s" -> JsString("s")))) else None)
